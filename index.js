...
function getArtistByIndex(array, index) {
    console.log(`The Artist at ${index} is ${array[index].name}`)
  }
...
function get20s(arr){
  let yearsArr = arr.filter(function (artist) {
    let yearsArr = artist.years.split(' ');
    let died = yearsArr[2];
    return (Number(died) >= 1900 && Number(died) <= 2000);
  })
  return yearsArr;
}
...
function addArtist(arr, id, name, years, genre, nationality, bio){
  ...
  addArtist(artists, 21, 'andrew', '1990 - living', 'varied', 'American', 'Lorem Ipsum Dolar Set')
...
function lotsOfArt(arr){
  ...
  console.log(arr.filter(arr => arr.paintings >= 100))
